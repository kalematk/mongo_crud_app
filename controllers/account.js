...
import Account from '../models/account.js';// schema  של account לקשר ל  
import bcryptjs from 'bcryptjs'; //Password crypt
import jwt from 'jsonwebtoken'; //manage token
    ...
    .then(async account =>{
        if(account){
            ...
            const _account = new Account({
                ...
                email:user.email,
                password :hash,
                verficationCode:generateRandomIntegerInRange(1000,9999),//password 4numbers
                mobile:user.mobile
            })
            _account.save()
            .then(accountCreated =>{
...
//password פןנקציה ל כמות המספרים ב password
